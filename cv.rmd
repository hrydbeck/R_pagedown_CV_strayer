...
if(params$pdf_mode){
  ...
  cat("[<i class='fas fa-download'></i> Download a PDF of this CV](https://github.com/halryd/R_pagedown_CV_strayer/blob/main/cv.pdf)")
}
```
...
```{r}
# Note the special double pipe so we modify the CV object in place
CV %<>% print_text_block("objective") 
```

```{r}
# Note the special double pipe so we modify the CV object in place
CV %<>% print_text_block("drivingForces") 
```
...
```{r}
# Note the special double pipe so we modify the CV object in place
CV %<>% print_text_block("relevantCompetences") 
```
